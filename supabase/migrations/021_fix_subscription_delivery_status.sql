-- Migration 021: Fix create_subscription_delivery to accept a status parameter
-- so auto-generated spread orders can be created as 'confirmed' instead of
-- immediately 'delivered'. Wallet debit still happens at creation (reservation).
-- Also backfills any future-dated subscription orders wrongly marked 'delivered'.

-- ── 1. Replace RPC with parameterised status ────────────────────────────────
...
create or replace function public.create_subscription_delivery(
  ...
  p_bowls jsonb,
  p_status text default 'delivered'   -- new param; existing callers unchanged
)
returns table (
  order_id uuid,
...
begin
  ...
  insert into public.orders (
    ...
    p_subscription_id,
    'subscription',
    p_status,              -- use the param instead of hardcoded 'delivered'
    p_delivery_date,
...
-- ── 2. Backfill: set future-dated subscription orders back to 'confirmed' ───
-- These were incorrectly created as 'delivered' during subscription approval.
