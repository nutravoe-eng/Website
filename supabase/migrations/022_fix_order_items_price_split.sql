-- Migration 022: Fix create_subscription_delivery to store unit_price as base
-- bowl price only, and total_price as (base + customization) × quantity.
-- This makes the invoice "Unit Price" and "Customisation" columns display correctly
-- using the existing formula: customCost = total_price - unit_price × qty.
...
begin
  ...
  -- so the split is preserved in order_items but the full amount is charged.
  select coalesce(sum(
  ...
  -- Store unit_price as base only; total_price = (base + customization) × qty.
  -- Invoice formula customCost = total_price - unit_price × qty gives customization correctly.
